...
tags = shuffle(pd.read_csv('./data/csv/tag_cat_results_new.csv', sep='\t'))
tags = tags.drop(tags[(tags['dbpDescription'].isna()) & (tags['wdDescription'].isna())].index)
tags['text'] = tags['dbpDescription'] + ' ' + np.where(~tags['wdDescription'].isna(), tags['wdDescription'], '')
...
GLOVE_DIR = "./data/txt/glove.6B.100d.txt"
embeddings_index = {}
f = open(GLOVE_DIR)
for line in f:
    try:
...
preds = Dense(9, activation='softmax')(sent_att)
